    ...
    <%else
{
    ...
    <%if (Model.noticia.Marked)
      {%>
    | <font face="Arial" color="#ff0000"><strong>Esta Noticia está a violar as regras e
        pode ser apagada a qualquer momento</strong> </font>
    <% }%>
    <fieldset>
        ...
        <p align="right">
            <small>
                <%=Html.Encode(Model.noticia.Data) %></small>
        </p>
        <%if (Request.IsAuthenticated)
          { %>
        <p align="right">
                ...
                <tr>
                    <td>
                        <fieldset>
                            ...
                            <center>
                                <p>
                                    <%= Html.Encode(Model.noticia.Tags)%></p>
                            </center>
                        </fieldset>
                    </td>
                </tr>
    ...
    <center>
        <big class="arrow"><b>
            <%:Html.ActionLink("⇧","UpVote","News",new { id = Model.noticia.NoticiaId.ToString()}, null)%></b></big>
        <h3>
            <%=Html.Encode(Model.noticia.Pontuacao)%></h3>
        <big class="arrow"><b>
            <%:Html.ActionLink("⇩", "DownVote", "News",new { id = Model.noticia.NoticiaId.ToString()}, null)%></b></big>
    </center>
    <%--<fieldset>--%>
    <%if (Model.comments == null)
      { %>
    <center>
        <b>Nao há comentarios para este noticia</b></center>
    <%}%>
    <%else
        {%>
        ...
        <tr>
            <td>
                <fieldset>
                    ...
                    <p align="right">
                        <small>
                            <%=Html.Encode(c.Jornalista.Nome) %>
                            <%:Html.ActionLink("+", "VerPerfil", "Account", new { email = c.Jornalista.Email}, null)%></small>
                    </p>
                    <p align="right">
                        <small>
                            <%=Html.Encode(c.Data) %></small>
                    </p>
                </fieldset>
            </td>
        </tr>
